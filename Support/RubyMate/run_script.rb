...
ruby = nil
rvm_path = "#{ENV["HOME"]}/.rvm/scripts/rvm"
rvmrc_path = "#{ENV["TM_PROJECT_DIRECTORY"]}/.rvmrc"

if File.exist?(rvm_path) && File.exist?(rvmrc_path)
  ruby = `. #{rvmrc_path} && which ruby`.strip
end

ruby ||= ENV['TM_RUBY'] || 'ruby'

cmd = [ruby, '-rcatch_exception']
